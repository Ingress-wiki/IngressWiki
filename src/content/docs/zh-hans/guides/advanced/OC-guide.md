...
校准通常需要 10 秒，如果扫描良好（包括建立的模型质量和 OC 时质量）可以缩短到 1-2 秒。在整个校准过程中，视频以 500-700KB/s 速度上传
...
是的，可以进行 OC，不进行结算且显示最终屏幕后将显示一个错误，并且您将需要重新进行 Hack。但是您没有剩余 XM 将无法进行 OC 。省流： XM 非空可进行 OC ，但如果 XM 小于 入侵所消耗的 XM 不进行结算。
